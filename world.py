...
class World():
    def __init__(self):
        self.map_tiles = []
        self.obstacle_tiles = []#set collision in the tiles, walls(number 7)
        self.exit_tile = None #set tile for changing level (number 8)
        self.item_list = []
        self.player = None
        self.character_list = []
    ...
    def process_data(self, data, tile_list, item_images, mob_animations):
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                ...
                #in this case the number 7 tile is the wall image
                if tile == 7:
                    self.obstacle_tiles.append(tile_data)
                #number 8 is the ladder image to change level
                elif tile == 8:
                    self.exit_tile = tile_data
                elif tile == 9:
                    coin = Item(image_x, image_y, 0, item_images[0])
                    self.item_list.append(coin)
                    tile_data[0] = tile_list[0]#paints the surface behind coins, potions and characters with a normal tile
                elif tile == 10:
                    potion = Item(image_x, image_y, 1, [item_images[1]])
                    self.item_list.append(potion)
                    tile_data[0] = tile_list[0]
                elif tile == 11:#set player
                    player = Character(image_x, image_y, 100, mob_animations, 0, False, 1)
                    self.player = player
                    tile_data[0] = tile_list[0]
                elif tile >= 12 and tile < 17:#set enemy between those image numbers
                    enemy = Character(image_x, image_y, 100, mob_animations, tile - 11, False, 1)
                    self.character_list.append(enemy)
                    tile_data[0] = tile_list[0]
                elif tile == 17:#exception for the final boss enemy
